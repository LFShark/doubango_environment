...
then
	...
	export ANDROID_TOOLCHAIN=$(pwd)/my-android-toolchain-android-armv7-a
    	$NDK/build/tools/make-standalone-toolchain.sh --arch=arm --platform=android-9 --install-dir=$ANDROID_TOOLCHAIN
    	export CFLAGS='-march=armv7-a -mfloat-abi=softfp -fPIC -shared'
...
then
	...
	export SYSROOT=$NDK/platforms/android-9/arch-arm
    	...
    	$NDK/build/tools/make-standalone-toolchain.sh --arch=arm --platform=android-9 --install-dir=$ANDROID_TOOLCHAIN
    	export CFLAGS='-march=armv7-a -mfloat-abi=softfp -mfpu=neon -fPIC -shared'
    	export LDFLAGS='-Wl,--fix-cortex-a8'
	export OS=android-armv7
else
	echo "************************"
