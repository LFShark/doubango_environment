...
do
	...
	./configure --host=$HOST --with-android-cpu=$arch --prefix=$HOME/armv7-a-neon --with-pic --disable-ffmpeg--enable-nonfree=$ENABLE_NONFREE --enable-gpl=$ENABLE_GPL --enable-debug=$DEBUG $OPTIONS
	make clean
	make uninstall
	make all CFLAGS='-fPIC -fpic -mfpu=neon -funsafe-math-optimizations'
	if [ $DEBUG = "yes" ]; then \
		make install; \
	else \
		make install-strip; \
...
mkdir -p $HOME/imsdroid/libs/armv7-a-neon
...
cp --force $HOME/armv7-a-neon/lib/libtinyWRAP.so.0.0.0 $HOME/imsdroid/libs/armeabi-v7a-neon/libtinyWRAP_neon.so
